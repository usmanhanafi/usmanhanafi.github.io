...
const BadgeIcon = ({ icon, children, size, className }) => {
    return (
        <div className={`${className ?? ''} ${size == 'small' ? 'px-3 py-1 font-semibold' : 'font-bold'} flex gap-2 items-center px-6 py-3 rounded-full bg-white/80 text-secondary-light hover:bg-white/90`}>
            <Image
